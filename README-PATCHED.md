...
| 文件 | 作用 |
|---|---|
| `patches/anytype-heart-discussion-id.patch` | 补丁，基线记录在 `patches/heart-patch-base`（当前 `v0.50.20`）：① GO-3132 discussionId API；② v1 ChatMessage 暴露 blocks（桌面端消息不再读出空文本）；③ v1 写入侧合成 text block（API 发的消息与桌面端同构，`blocks` 成为所有消息的统一读取入口）；④ 新端点 `POST /v1/spaces/{space_id}/objects/{object_id}/discussion`（agent 可自行为无讨论的对象冷启动讨论） |
